...
function animatedForm() {
  ...
  $('.fa-arrow-circle-down.email').click(function () {
    ...
    if (validateEmail(email)) {
      ...
      if (url == 3) {
        $('#prompt').text(
          name + ' ơi, ' + name + ' hứng thú về lĩnh vực nào nhỉ?'
        );
        $('#like-input').val('Tất cả mọi lĩnh vực!');
      } else {
        $('#prompt').text(
          'Which areas of study are you interested in, ' + name + '?'
        );
  ...
  $('.fa-arrow-circle-down.like').click(function () {
    ...
    if (url == 3) {
      $('#prompt').text(
        'Cuối cùng, ' + name + ' có muốn gửi mình một lời nhắn không?'
      );
      $('#message-input').val('Đi ăn chè với ' + name + ' nha Calum ơi!');
    } else {
      $('#prompt').text(
        'Lastly, would you like to send me a message, ' + name + '?'
      );
